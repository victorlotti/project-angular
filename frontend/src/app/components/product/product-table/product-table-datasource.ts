...
export class ProductTableDataSource extends DataSource<Product> {
  ...
  private getSortedData(data: Product[]): Product[] {
    ...
    return data.sort((a, b) => {
      ...
      return compare(a, b, isAsc, this.sort);
    });
  }
}

/** Simple sort comparator for example ID/Name columns (for client-side sorting). */
function compare(a: Product, b: Product, isAsc: boolean, sort: MatSort | undefined): number {
  switch (sort?.active) {
    case 'name': 
    ...
    case 'id': 
      if (!a.id && b.id){
        ...
        return ((a.id ? a.id : 0) < (b.id ? b.id : 0) ? -1 : 1 * (isAsc ? 1 : -1));
      }
